...
type
  ...
  protected
    ...
    function PosEx(ch: char; const s: string; offset: integer = 1): integer;
...
function TXMLXPathEvaluator.PosEx(ch: char; const s: string; offset: integer = 1): integer;
begin
...
begin
  ...
  if pOp = 0 then begin
    ...
    left := Copy(predicate, 1, pOp-1);
    op := predicate[pOp];
    right := Copy(predicate, pOp+1, Length(predicate)-pOp);
