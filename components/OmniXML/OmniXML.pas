{ $OmniXML: OmniXML/OmniXML.pas,v 1.17 2005/11/02 13:57:20 mremec Exp $ }
...
//    function CloneNode(const Deep: Boolean): IXMLNode; override;
  ...
  private
    procedure CheckValue(const Value: WideString);
  protected
    procedure SetData(const Value: WideString); override;
    procedure SetNodeValue(const Value: WideString); override;
    procedure InternalWriteToStream(const OutputStream: IUnicodeStream); override;
    procedure ReadFromStream(const Parent: TXMLNode; const InputStream: IUnicodeStream); override;
  public
...
procedure TUnicodeStream.WriteString(const Value: WideString);
begin
  if Value <> '' then
...
{$ENDIF}
{$IFDEF LINUX}
  if FStreamMode = smRead then begin
...
begin
  ...
  if Deep and HasChildNodes then begin
    // clone child nodes
...
begin
  ...
  if (RefChild <> nil) and (FChildNodes.IndexOf(RefChild) = -1) then
  ...
  if RefChild = nil then
    ...
    Index := FChildNodes.IndexOf(RefChild);
...
begin
  ...
  while i < Attributes.Length do begin
    if Attributes.Item[i].NodeName = Name then begin
...
begin
  ...
  OutputStream.WriteString('<' + FNodeName);
  ...
  if not HasChildNodes then begin
...
begin
  // 2002-12-17 (mr): fixed indentation
  OutputStream.WriteIndent;
  OutputStream.WriteString('<?' + FNodeName + ' ' + FData + '?>');
end;
...
begin
  OutputStream.WriteString(' ' + FNodeName + '="' + EncodeText(NodeValue) + '"');
end;
...
procedure TXMLCDATASection.SetData(const Value: WideString);
begin
  CheckValue(Value);
  inherited;
end;
...
var
  ...
  chEndOfString: WideChar;
  PState: TParserState;
  ReadChar: WideChar;
  skipInitialSpace: boolean;
begin
  bracketDepth := 0;
  chEndOfString := #0; // to keep Delphi happy
  ...
  while InputStream.ProcessChar(ReadChar) do begin
    case PState of
      psData: begin
        if skipInitialSpace then begin
          if CharIs_WhiteSpace(ReadChar) then
            continue
          else
            skipInitialSpace := false;
        end;
        if (ReadChar = '>') and (bracketDepth <= 0) then begin
          NodeValue := InputStream.GetOutputBuffer;
          Exit;
        end
        else begin
          ...
          if ReadChar = '[' then
            ...
            Dec(bracketDepth)
          else if (ReadChar = '"') or (ReadChar = '''') then begin
            PState := psInString;
            chEndOfString := ReadChar;
          end;
        end;
      end; //psData
      psInString: begin
        InputStream.WriteOutputChar(ReadChar);
        if ReadChar = chEndOfString then
          PState := psData;
      end; //psInString
    end; //case
  end;
...
begin
  ...
  if HasChildNodes then begin
    i := 0;
    while (Result = nil) and (i < ChildNodes.Length) do begin
...
function TXMLDocument.InternalCreateDocType(const Data: WideString): TXMLDocumentType;
begin
  Result := FXMLDocTypeClass.CreateDocumentType(Self, Data);
end;

function TXMLDocument.CreateDocType(const Data: WideString): IXMLDocumentType;
begin
